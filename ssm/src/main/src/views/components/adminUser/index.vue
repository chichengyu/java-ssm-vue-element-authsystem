    ...
    data () {
        return {
            ...
            tableData: {
                ...
                // 分页
                page:{align:'right',total:1,size:pageOffset,currentPage:1,currentChange:(currentPage) => {
    ...
    methods: {
        ...
        labelInit(){
            this.tableData.tableLabel = [
                ...
                {prop:'telephone',title:'手机'},
                {prop:'mail',title:'邮箱'},
                {prop:'deptId',title:'部门'},
                ...
                {prop:'status',title:'状态',isSwitch:true,width:80,style:(params,item)=>{return params.row.id==1?{display:'none'}:''},change:(params) => {
                    let txt = params.row.status==1?'正常':'禁用';
        ...
        tableDataInit(currentPage,pageOffset,keywords='') {
            ...
            getTableData(currentPage, pageOffset, keywords).then(res => {
                if (res.data.code == 1) {
                    this.tableData.tableData = res.data.data.list;
